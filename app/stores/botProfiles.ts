...
export const useBotProfilesStore = defineStore('botProfiles', () => {
  // State
  const profiles = ref<BotProfile[]>([])
  const activeProfileId = ref<string | null>(null)

  // Cookie persistence
  const profilesCookie = useCookie<BotProfile[]>('telegram-bot-profiles', {
    default: () => [],
    maxAge: 60 * 60 * 24 * 365, // 1 year
  })

  const activeProfileCookie = useCookie<string | null>('telegram-active-profile', {
    default: () => null,
    maxAge: 60 * 60 * 24 * 365, // 1 year
  })
  ...
  // Actions
  function addProfile(profile: Omit<BotProfile, 'id'>) {
  ...
  function updateProfile(id: string, updates: Partial<Omit<BotProfile, 'id'>>) {
    ...
    if (index !== -1) {
      ...
      if (current) {
        profiles.value[index] = {
          id: current.id,
          bot_name: updates.bot_name ?? current.bot_name,
          bot_token: updates.bot_token ?? current.bot_token,
        }
      }
    }
  }
